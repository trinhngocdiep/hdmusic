(function() {
    ...
    _players.forEach(e => {
        ...
        e.onPause = function() {
            player.onPause && player.onPause(state.currentTrack);
        };
        e.onProgress = function(time) {
            player.onProgress && player.onProgress(state.currentTrack, time);
        };
        e.onEnd = function() {
            next();
        };
    ...
    function resume() {
        if (_currentPlayer) {
            _currentPlayer.resume(state.currentTrack);
        }
    }
...
YtPlayer.prototype.play = function(track) {
    ...
    if (this.player.initiated) {
        this.player.loadVideoById && this.player.loadVideoById(this.track.videoId);
    } else {
        this.player = new YT.Player(this.containerId, {
            ...
            events: {
                ...
                onError: () => {
                    console.log('cannot play video', this.track.title);
                    this.track.error = true;
                    this.onEnd && this.onEnd();
                }
            }
        });
...
YtPlayer.prototype.pause = function() {
    this.player.initiated && this.player.pauseVideo();
};
...
if (!YT.loading) {
    ...
    (function () {
        ...
        //a.src = 'https://s.ytimg.com/yts/jsbin/www-widgetapi-vflK6YIUJ/www-widgetapi.js';
        //a.async = true;
